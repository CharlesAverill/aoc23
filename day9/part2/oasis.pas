program Oasis;
(*
    Given sequences of values over time, extrapolate each sequence backwards
    by 1 and return the sum of the extrapolated values. Extrapolation is performed 
    by calculating the rates of change in each pair of a sequence, then
        - if the rates of change are all 0, prepend a 0 to the sequence, and
          prepend an extrapolated value to the sequence above by doing
            upper_seq = [first(upper_seq) - first(current_seq)] + upper_seq
...
(* Shift all elements right by one and insert a value at the start *)
function shift_right(seq: array of cardinal; value: cardinal) : cardinal_array;
var
    i: cardinal;
begin
    setlength(shift_right, length(seq) + 1);
    shift_right[0] := value;
    for i := 0 to length(seq) - 1 do
        shift_right[i + 1] := seq[i];
end;
...
begin
    ...
    begin
        ...
        for i := high(derivatives) - 1 downto 0 do
            derivatives[i] := shift_right(derivatives[i], derivatives[i][0] - derivatives[i + 1][0]);

        sum := sum + derivatives[0][0];
    end;
